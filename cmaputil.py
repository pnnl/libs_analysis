...
rgb = np.loadtxt('cividis.txt').T
cmap = colors.ListedColormap(rgb.T, name='cividis')
...
def _check_cmap(cmap):
    ...
    if cmap is None or cmap not in CMAPS:
        if not exists(cmap + '.npy') and not exists(cmap + '.txt'):
...
def cdps_plot(img, cmap, rgb, num, gslope, name=None):
